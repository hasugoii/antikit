...
面对复杂bug时，使用 **5 Whys** 技术找到根本原因:

```
🔍 5 Whys 示例:
❓ 为什么应用崩溃? → 因为数据返回null
❓ 为什么是null? → 因为API没有返回数据
❓ 为什么没返回? → 因为用户在DB中没有记录
❓ 为什么没记录? → 因为注册流程跳过了初始化步骤
❓ 为什么跳过? → 因为验证没有捕获这种情况
✅ 根本原因: 注册流程中缺少验证
```
...
| 严重程度 | 描述 | 行动 |
|----------|------|------|
| 🔴 CRITICAL | 崩溃、数据丢失、安全 | 立即修复 |
| 🟠 MAJOR | 核心功能不工作 | 今天修复 |
| 🟡 MINOR | 次要功能出问题、UI问题 | 本迭代修复 |
| ⚪ TRIVIAL | 小问题、错别字、外观 | 有时间就修 |

### 2.6. 检查以前的Bug模式
*   在寻找新解决方案之前，检查:
    *   `session.json` → errors_encountered (本次会话修复的bug)
    *   `knowledge/` → 学到的模式
*   "我看到这个bug类似于之前修复的 [X]。让我尝试同样的方法..."
...
1️⃣ 运行 /test 彻底测试
2️⃣ 还有错误? 继续 /debug
3️⃣ 修复后坏了更多? /rollback
4️⃣ 都好了? /save-brain 保存
```
