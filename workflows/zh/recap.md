...
| # | 阻塞项 | 类型 | 等待天数 | 行动 |
|---|--------|------|---------|------|
| 1 | API缺少响应格式 | 外部 | 2天 | 联系后端团队 |
| 2 | 支付测试失败 | 技术 | 1天 | 需要 /debug |

💡 建议: #2是可以自己修复的技术问题，先解决它。
```

### 4.3. 自动检测阻塞
AI自动检测潜在阻塞:
- `session.json` → errors_encountered (未修复的bug)
- `session.json` → pending_tasks 状态为 "blocked"
- 包含 "WIP", "TODO", "FIXME" 的最近提交
- 正在跳过或失败的测试
